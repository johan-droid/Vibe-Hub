...
describe('tenant isolation, DLP, prompt confidentiality, and edge controls', () => {
  ...
  it('documents and enforces edge/control-plane protection settings', async () => {
    ...
    const terraform = await fs.readFile(path.join(repoRoot, 'ops/terraform/aws/main.tf'), 'utf-8');
    const k8sPolicy = await fs.readFile(path.join(repoRoot, 'ops/kubernetes/base/network-policies.yaml'), 'utf-8');
    expect(terraform).toContain('AWSManagedRulesBotControlRuleSet');
    expect(terraform).toContain('aws_shield_protection');
    expect(k8sPolicy).toContain('control-plane-internal-only');
  });
});
