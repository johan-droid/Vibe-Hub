...
async function evaluateFrontendBehaviorChange(task, context) {
  const build = await context.runRootNpm('ui-build', ['run', 'build', '--workspace=frontend']);
  const chatInterfaceSource = await context.readFile('frontend/src/features/chat/components/ChatInterface.jsx');
  ...
  const extraEvidence = [
    ...
    evidence('source', 'Workspace upload flow', hasHarnessHook ? 'passed' : 'failed', hasHarnessHook
      ...
      : 'Harnessing hook is missing from the frontend upload flow.', {
        file: 'frontend/src/features/chat/components/ChatInterface.jsx',
      }),
  ];
...
async function evaluateFrontendHostileOutput(task, context) {
  const chatSource = await context.readFile('frontend/src/features/chat/components/ChatInterface.jsx');
  const terminalSource = await context.readFile('frontend/src/features/editor/components/Terminal.jsx');
  const diffSource = await context.readFile('frontend/src/features/editor/components/DiffViewer.jsx');
