...
describe('triage router', () => {
  ...
  it('classifies bug-fix style prompts deterministically and extracts file hints', async () => {
    const result = await triageAndRoute('Fix the upload crash in backend/orchestrator/router.js and tighten the handler.');
    ...
    expect(result.target_files).toContain('backend/orchestrator/router.js');
  });
