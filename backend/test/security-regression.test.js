...
describe('security regression: sandbox', () => {
  it('documents networkless explicit-copy sandbox semantics in the tool contract', async () => {
    const toolsSource = await fs.readFile(path.join(REPO_ROOT, 'backend/orchestrator/tools.js'), 'utf-8');
...
describe('security regression: vfs-paths', () => {
  it('rejects similarly prefixed sibling paths', () => {
    const vfs = new VFSContainer({ workDir: path.join(REPO_ROOT, 'backend') });
...
describe('security regression: frontend-xss', () => {
  it('keeps raw HTML rendering and terminal output behind React escaping boundaries', async () => {
    const uiFiles = [
      'frontend/src/features/chat/components/ChatInterface.jsx',
      'frontend/src/features/editor/components/Terminal.jsx',
      'frontend/src/features/editor/components/DiffViewer.jsx',
    ];
